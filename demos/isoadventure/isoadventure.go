	...
	for _, tileLayer := range levelData.TileLayers {
		for _, tileElement := range tileLayer.Tiles {
			...
			if tileElement.Image != nil {
				...
				//zo := float32(128)
				zo := tile.RenderComponent.Drawable.Height() - float32(levelData.TileHeight)

				tile.RenderComponent.SetZIndex(tileElement.Point.Y + zo)
...
func (s *SpeedSystem) Update(dt float32) {
	...
	for _, e := range s.entities {
		...
		e.RenderComponent.SetZIndex(e.SpaceComponent.Position.Y + eh)
	}
